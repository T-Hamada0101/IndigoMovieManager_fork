...
WHERE QueueId = @QueueId;";
    ...
    [Test]
    public void Done行へ同条件Upsertが来てもDoneを維持する()
    {
        string mainDbPath = Path.Combine(
            Path.GetTempPath(),
            $"imm-main-done-keep-{Guid.NewGuid():N}.wb"
        );
        QueueDbService queueDbService = new(mainDbPath);
        string queueDbPath = queueDbService.QueueDbFullPath;
        DateTime nowUtc = DateTime.UtcNow;
        ...
        string moviePath = Path.Combine(
            Path.GetTempPath(),
            $"movie-done-keep-{Guid.NewGuid():N}.mp4"
        );
        ...
        try
        {
            ...
            using SQLiteCommand verifyCommand = verifyConnection.CreateCommand();
            verifyCommand.CommandText = @"
SELECT Status, AttemptCount, OwnerInstanceId, LeaseUntilUtc, StartedAtUtc
FROM ThumbnailQueue
WHERE TabIndex = 0;";
            using SQLiteDataReader reader = verifyCommand.ExecuteReader();
            Assert.That(reader.Read(), Is.True);
            Assert.Multiple(() =>
            {
                Assert.That(reader.GetInt32(0), Is.EqualTo((int)ThumbnailQueueStatus.Done));
                Assert.That(reader.GetInt32(1), Is.EqualTo(0));
                Assert.That(reader.IsDBNull(2) ? "" : reader.GetString(2), Is.EqualTo(""));
                Assert.That(reader.IsDBNull(3) ? "" : reader.GetString(3), Is.EqualTo(""));
                Assert.That(reader.IsDBNull(4) ? "" : reader.GetString(4), Is.EqualTo(""));
            });
        }
    ...
    [Test]
    public void Done行でも条件差分があればPendingへ戻して再作成できる()
    {
        string mainDbPath = Path.Combine(
            Path.GetTempPath(),
            $"imm-main-done-rebuild-{Guid.NewGuid():N}.wb"
        );
        QueueDbService queueDbService = new(mainDbPath);
        string queueDbPath = queueDbService.QueueDbFullPath;
        DateTime nowUtc = DateTime.UtcNow;
        ...
        string moviePath = Path.Combine(
            Path.GetTempPath(),
            $"movie-done-rebuild-{Guid.NewGuid():N}.mp4"
        );
        string moviePathKey = QueueDbPathResolver.CreateMoviePathKey(moviePath);
        ...
        try
        {
            ...
            using SQLiteCommand verifyCommand = verifyConnection.CreateCommand();
            verifyCommand.CommandText = @"
SELECT Status, AttemptCount, MovieSizeBytes, OwnerInstanceId, LeaseUntilUtc, StartedAtUtc
FROM ThumbnailQueue
WHERE TabIndex = 0;";
            using SQLiteDataReader reader = verifyCommand.ExecuteReader();
            Assert.That(reader.Read(), Is.True);
