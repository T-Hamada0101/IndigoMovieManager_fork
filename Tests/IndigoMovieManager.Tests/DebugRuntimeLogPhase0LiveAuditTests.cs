...
public sealed class DebugRuntimeLogPhase0LiveAuditTests
{
    ...
    private static void FailWithSummary(string reason, DebugRuntimeLogAuditSummary summary)
    {
        Assert.Fail(
            string.Join(
                Environment.NewLine,
                reason,
                $"実機操作で同一 Release run の {DebugRuntimeLogPhase0NextActionPolicy.BuildFullCaptureGuideText()} を採取してから再実行してください。",
    ...
    private static string[] AllEvidenceMessages()
    {
        ...
        [
            ...
            "detail image_contract=image-pipeline-v1 failure_reason=stale-player-right-rail",
            "save persist_contract=persistence-write-v1",
