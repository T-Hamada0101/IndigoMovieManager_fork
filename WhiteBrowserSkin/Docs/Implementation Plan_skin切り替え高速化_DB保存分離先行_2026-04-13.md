...
変更概要:
- 2026-06-18 のサブEで、skin profile write の失敗時表現を一段進めた。`WhiteBrowserSkinProfileValueCache` は pending / persisted / faulted に加えて `dirty / failed / retryable` の状態 snapshot を返し、queue / persister / fallback 失敗時の `skin-db` ログにも `dirty=true failed=true retryable=true` を出す。UI hot path は `TryEnqueueExternalSkinProfileWrite(...)` の enqueue のみに保つ source policy で固定した。
...
- 検証用 worktree は `%USERPROFILE%\source\repos\IndigoMovieManager-worktree-*` のような sibling ディレクトリへ作る。
...
- 明示 reload では現在外部 skin 定義を再確認し、通常 refresh やヘッダー表示同期では不要な catalog 署名確認を増やさない
...
- 2026-06-18: `WhiteBrowserSkinProfileValueCache` に `dirty / failed / retryable` の状態 snapshot を追加した。profile 保存失敗時は失敗値を保持し、API 表示値や初期タブ復元には使わず、再保存可能な dirty として cache / log でだけ読めるようにした
...
## 12. 2026-04-24 時点の練り直し後の実施順

skin 本線は、runtime bridge 側の大半が固定済みになった。
ここからは「ケースを足して面で押す」段階ではなく、共通原因を潰して、MainWindow 実 host で本当に必要な受け入れだけを残す段階へ切り替える。

### 12.1 最優先: MainWindow teardown / fail-fast 共通原因

- `MS.Win32.HwndSubclass.SubclassWndProc` 起点の focused 終了時 crash を、個別 skin の失敗ではなく MainWindow 実 host の終了処理共通問題として扱う
- `WebView2` / host window / overlay / dispatcher shutdown の順序を確認し、閉じる前に callback / refresh / pending navigate が残らない状態へ寄せる
- まずは代表 1 件で、テスト assertion 自体は通るが終了時に落ちるケースを再現対象にする
- ここが揺れている間は、MainWindow 実 host の新規ケースを大量追加しない

### 12.2 次点: bare `TagInputRelation` runtime bridge 直列

- 残る runtime bridge 側の主戦場は、bare `TagInputRelation` の `onSkinLeave/onClearAll -> MissingSkin -> #umlFindTreeEve` に絞る
- 先に build 出力 skin で効果があった host 側 `messageId / request / resolve` 追跡へ観測を寄せ、ページ内 state 消失による擬似 timeout を切り分ける
- 観測を安定化しても timeout する場合だけ、compat bridge / `changeSkin` pending / navigate 順序の本体修正へ進む
- `Get後` と `Save後` は green のため、再調査対象へ戻さない

### 12.3 第三優先: build 出力 skin 4 本の MainWindow 受け入れ判断

- build 出力 skin 4 本 (`DefaultSmallWB / Chappy / Search_table / Alpha2`) は、runtime bridge 側では `tag / thumb` の terminal rerender と `MissingSkin -> success` 直列まで固定済み
- MainWindow 実 host 側は、teardown / fail-fast 共通原因を先に直してから再判定する
- 再判定後も runtime bridge 正本だけで十分に言い切れるものは、MainWindow 側へ無理に複製しない
- ユーザー操作に近い代表ケースだけを MainWindow 受け入れとして残す

### 12.4 第四優先: 体感テンポの受け入れ

- `refresh end` の `elapsed_ms / outcome / catalog_* / persist_*` を、外部 skin 連打と代表 skin 往復で読み、体感コストが悪化していないことを確認する
- `refresh / stale / catalog / persist` のどこが支配的か、`debug-runtime.log` だけで追える状態を維持する
- DB 分離は土台施策として維持し、体感改善の主語は `refresh / stale / catalog / navigate` の削減に置く
...
この 2 本は、次の 4 区分で読める状態へ保つ。

- 固定済み
- MainWindow teardown / fail-fast 待ち
- runtime bridge の bare `TagInputRelation` 待ち
- runtime bridge 正本だけで十分と判断したもの
...
- MainWindow 実 host の focused 終了時 fail-fast が、代表ケースで再現しない、または発生条件と回避線が docs で説明できる
- bare `TagInputRelation` の runtime bridge `MissingSkin -> #umlFindTreeEve` が green になる、または本体修正が必要な理由を `messageId / request / resolve` 追跡で説明できる
- build 出力 skin 4 本の MainWindow 実 host 受け入れ範囲が、runtime bridge 正本との役割分担込みで決まっている
- docs だけ読めば、固定済み・MainWindow 共通問題・runtime bridge 残差・意図的に複製しない領域が区別できる
...
- 2026-04-23 追記: `da6a22b` と `0a9fd64` で compat alias fallback と lifecycle pending 応答を補強したが、これは pending 解決の取りこぼし防止である。2026-04-24 時点では、runtime bridge 側の直列 timeout は bare `TagInputRelation` に限定して扱う。
...
- `TagInputRelation` の runtime bridge 直列は、2026-04-24 時点で次の切り分けを正本とする
