...
namespace IndigoMovieManager.UpperTabs.Common
{
    ...
    internal static class ImageLoadLogFields
    {
        ...
        {
            return
                $"image_contract=image-pipeline-v1 image_role={result.ImageRequest.ThumbnailRole} image_request_revision={result.ImageRequest.RequestRevision} image_key={result.ImageRequest.MoviePathKey ?? ""} visible_priority={FormatLogBool(result.ImageRequest.IsVisiblePriority)} image_cache_policy={result.ImageRequest.CachePolicy} should_decode={FormatLogBool(result.ImageRequest.ShouldDecode)} {BuildStateSuffixCore(result, includeOutcome: true)}";
        }
