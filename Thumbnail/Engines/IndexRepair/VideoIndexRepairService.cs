...
{
    /// <summary>
    /// FFmpeg.AutoGen で動画インデックスの判定/修復を行うサービス。
    /// </summary>
    internal sealed class VideoIndexRepairService : IVideoIndexRepairService
    {
        ...
        private static bool EnsureFfmpegInitializedSafe(out string errorMessage)
        {
            ...
            lock (_initLock)
            {
                ...
                try
                {
                    // 既存の autogen と同じ探索順で shared DLL の配置先を解決する。
        ...
        private static VideoIndexProbeResult ProbeInternal(string moviePath, CancellationToken cts)
        {
            ...
            // 通常オープンが失敗し、IGNIDX付きで成功するケースを「インデックス破損疑い」と判定する。
            ...
            // ストリーム情報取得のみ失敗し、IGNIDXで復帰した場合も同様に判定する。
            if (
        ...
        // 無劣化の再MUX（-c copy相当）でコンテナ構造を組み直す。
        ...
        private readonly struct ProbeOpenResult(
            bool IsSuccess,
            string Reason,
            string ErrorCode,
            string ContainerFormat,
        ...
        )
        {
            ...
            public string ContainerFormat { get; } = ContainerFormat ?? "";
            public int FindStreamInfoErrorCode { get; } = FindStreamInfoErrorCode;
        }
    }
}
