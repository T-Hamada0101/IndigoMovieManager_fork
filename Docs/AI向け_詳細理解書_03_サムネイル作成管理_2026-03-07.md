# AI向け 詳細理解書 03: サムネイル作成管理（投入/永続化/消費）

最終更新日: 2026-03-12

## 1. この機能の責務

- 作成対象ジョブの投入（Producer）
- QueueDBへの永続化（Persister）
- QueueDBからのリース取得と生成実行（Consumer）
- 成功/失敗/再試行の状態管理

## 2. 主要ファイル

- `Thumbnail/MainWindow.ThumbnailQueue.cs`
- `src/IndigoMovieManager.Thumbnail.Queue/QueuePipeline/QueueRequest.cs`
- `src/IndigoMovieManager.Thumbnail.Queue/QueuePipeline/ThumbnailQueuePersister.cs`
- `Thumbnail/MainWindow.ThumbnailCreation.cs`
- `src/IndigoMovieManager.Thumbnail.Queue/ThumbnailQueueProcessor.cs`
- `src/IndigoMovieManager.Thumbnail.Coordinator/ThumbnailCoordinatorHostService.cs`
- `src/IndigoMovieManager.Thumbnail.ProgressViewer/ThumbnailProgressViewerWindow.xaml.cs`

## 3. 全体パイプライン

```mermaid
flowchart TD
    A[TryEnqueueThumbnailJob] --> B[0KB判定/デバウンス]
    B --> C[QueueRequestへ変換]
    C --> D[Channel Writerへ投入]
    D --> E[ThumbnailQueuePersister.RunAsync]
    E --> F[短周期窓でバッチ化]
    F --> G[同一キーを最新1件へ圧縮]
    G --> H[QueueDbService.Upsert]
    H --> I[QueueDB Pending化]
    I --> J{Coordinatorモードか}
    J -- No --> K[ThumbnailQueueProcessor.RunAsync]
    J -- Yes --> L[CoordinatorがWorker群を起動]
    L --> K
    K --> M[Lease取得 + 並列CreateThumbAsync]
    M --> N{生成結果}
    N -- Success --> O[Done更新 + 進捗通知]
    N -- Fail --> P[再試行orFailed更新]
```

## 4. Producer側の設計意図

- 入力停止フラグにより終了シーケンスで新規投入を遮断。
- 0KB動画は早期除外し、エラーマーカーを置いて再投入ループを防止。
- デバウンスは監視イベントの連打吸収が目的。手動操作由来ジョブは抑止対象外。

## 5. Persister側の設計意図

- Channel単一Readerで順序性を保ちつつ、100-300ms窓で書込回数を削減。
- 同一判定キー（`MoviePathKey + TabIndex`）で最新要求のみ残し、無駄なUpsertを抑制。
- Supervisorで再起動可能にし、一時障害でパイプライン全停止しない設計。

## 6. Consumer側の設計意図

- QueueDBリースで多重実行時の取り合いを抑制。
- 並列度は設定値と実行時状況で調整し、詰まりを軽減。
- `CreateThumbAsync` 失敗は例外化し、Queue層で再試行/Failedへ正規遷移させる。
- 現行は本体Consumerだけでなく、Coordinator + Worker 構成へ切り替えられる。
- 進捗表示は `ProgressViewer` が別窓で受け持つ前提がある。
