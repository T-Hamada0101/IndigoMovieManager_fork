...
flowchart TD
    ...
    J --> K[CheckThumbAsyncがConsumer起動]
    K --> L[ThumbnailQueueProcessor.RunAsync]
    L --> M[Lease取得して並列CreateThumbAsync]
    M --> N[成功: Done更新 / 失敗:再試行またはFailed]
```
...
3. **消費段階（Consumer）**
   - `CheckThumbAsync` が `ThumbnailQueueProcessor.RunAsync` を常駐実行。
