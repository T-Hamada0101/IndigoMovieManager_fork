...
- `Views/Main/MainWindow.InputRouting.cs` へ IME 入力、一覧キー dispatch、sort combo、left drawer toggle を分離した。
- `Views/Main/DockLayoutRestorePolicy.cs` へ dock layout validation の純粋判断を分離した。
- `Views/Main/RegisteredMovieCountRefreshPolicy.cs` へ登録動画総数 refresh 結果の stale guard を分離した。
- `Views/Main/SortComboSelectionPolicy.cs` へ sort combo 選択変更時の経路判断を分離した。
- `Views/Main/MainDbSwitchPlanPolicy.cs` へ MainDB 切替時の副作用 plan 判断を分離した。
- `Views/Main/MainWindow.xaml.cs` は約 766 行まで縮み、constructor、不可避の Window event 配線、常駐 loop、互換 helper が中心になった。
...
- Registered movie count refresh policy
- Sort combo selection policy
- Main DB switch side effect plan policy
...
- Phase 1 から Phase 5 で、一覧要求制御、表示レコード生成、DB runtime、起動 / dock layout / window lifecycle、入力 routing は partial 分離済み。
- Phase 6 の最初の切り出しとして、dock layout validation policy は `DockLayoutRestorePolicy` へ分離済み。
...
- `ComboSort_SelectionChanged(...)` は当時 XAML event 入口として `MainWindow.xaml.cs` に残したが、Phase 5 で `MainWindow.InputRouting.cs` へ移動済み。`RefreshSelectionDetailAfterCollectionApplyIfNeeded(...)` は互換 refresh 入口として `MainWindow.xaml.cs` に残す。
...
- Phase 5 は完了済み。次は Phase 6 の候補監査として、純粋判断だけを WPF 非依存 helper へ出せるかを小さく見極める。

## 14. 2026-06-18 Phase 5 実施結果

実施内容:

- `Views/Main/MainWindow.InputRouting.cs` を追加し、`MenuToggleButton_Checked(...)` / `MenuToggleButton_Unchecked(...)`、`OnPreviewTextInput(...)` / `OnPreviewTextInputStart(...)` / `OnPreviewTextInputUpdate(...)`、`Tab_PreviewKeyDown(...)`、`ComboSort_SelectionChanged(...)` を `MainWindow.xaml.cs` から分離した。
- XAML handler 名と constructor の TextComposition handler 登録名は変更していない。
- `ComboSort_SelectionChanged(...)` の段階ロード中 `FilterAndSort(id.ToString(), true)` fallback、通常時 `await SortDataAsync(id.ToString())`、`id == "28"` の `RefreshThumbnailErrorRecords(force: true)`、`shouldSelectFirstItem` の `SelectFirstItem()` は維持した。
- `Tab_PreviewKeyDown(...)` は routing のみを持ち、`PlayMovie_Click(...)`、`TagEdit_Click(...)`、`MenuScore_Click(...)`、`RenameFile_Click(...)`、`OpenParentFolder_Click(...)`、`TryHandleDeleteShortcut(...)` の実処理は既存 partial のままにした。
- `FilterAndSort(..., true)`、直書き `Refresh();`、`Items.Refresh()` の許容線は増やしていない。

親レビュー:

- 実装レビューでは、`ComboSort_SelectionChanged(...)` の参照先を `MainWindow.InputRouting.cs` に追従し、`FilterAndSort(..., true)` の許容 2 箇所を新ファイル名で固定する必要を確認した。
- 全体観点レビューでは、コード側の Phase 5 は方針どおりで、正本ドキュメントを Phase 5 完了状態へ追従することを P1 とした。
- シンプル実装レビューでは、service / manager / DI を増やさず partial 分離だけに留め、`ComboSort_SelectionChanged(...)` と `Tab_PreviewKeyDown(...)` をこれ以上抽象化しない判断で妥当とした。

検証:

- focused test: 197 件成功。
- Release x64 build: 成功。`NETSDK1206` 警告 2 件は既存の SQLitePCLRaw RID 警告。
- `git diff --check`: 成功。
- 更新ドキュメントと新規 partial は UTF-8 BOMなし + LF。
...
- Phase 6 として、まず WPF 非依存へ出せる純粋判断の候補を監査する。大きな service graph や `MainWindow` 代替 core は作らない。

## 15. 2026-06-18 Phase 6-A 実施結果

実施内容:

- `Views/Main/DockLayoutRestorePolicy.cs` を追加し、dock layout validation の純粋判断を WPF 非依存 helper へ分離した。
- `FindMissingRequiredDockLayoutReason(...)`、`ShouldRequireThumbnailErrorBottomTab(...)`、`ContainsContentId(...)` だけを helper へ移した。
- `Views/Main/MainWindow.DockLayout.cs` は layout file read / backup / deserialize / AvalonDock repair を持つ View 境界として維持し、validation 呼び出しだけ `DockLayoutRestorePolicy` へ委譲した。
- `BottomTabs/ThumbnailError/MainWindow.BottomTab.ThumbnailError.cs` の既存 wrapper は残し、`ShouldShowThumbnailErrorBottomTab` の評価は MainWindow 側で行い、helper には bool を渡す契約を維持した。
- `DockLayoutRestorePolicy` は `System.Windows` / `AvalonDock` / `Dispatcher` / `LayoutAnchorable` / `File` / `Path` / `DebugRuntimeLog` / `MainVM` / `ObservableCollection` を参照しない source policy で固定した。

親レビュー:

- 全体観点レビューでは、`DockLayoutRestorePolicy` は `internal static` の純粋 helper に留まり、MainWindow 側も薄い互換 wrapper になっているため P0/P1 なしとした。
- シンプル実装レビューでは、移す最小単位を validation / thumbnail error bottom tab 必須判定 / content id 判定までに留め、backup、ログ、AvalonDock 探索、repair は `MainWindow.DockLayout.cs` に残す判断で妥当とした。
- 親レビューでは、XML parser 化、DTO / enum 化、`DockLayoutManager`、ApplicationCore project 作成は今回は過剰として退けた。

検証:

- focused test: 210 件成功。
- Release x64 build: 成功。`NETSDK1206` 警告 2 件は既存の SQLitePCLRaw RID 警告。
- `git diff --check`: 成功。
- 更新ドキュメントと新規 helper は UTF-8 BOMなし + LF。

次の推奨:

- Phase 6-D まで完了済み。次は抜本 UI 分割ロードマップの完了監査へ進む。

## 16. 2026-06-18 Phase 6-B 実施結果

実施内容:

- `Views/Main/RegisteredMovieCountRefreshPolicy.cs` を追加し、登録動画総数 refresh 結果を UI に反映してよいかの純粋判断を WPF 非依存 helper へ分離した。
- `RefreshRegisteredMovieCountAsync(...)` は、DB 読込、Dispatcher apply、`MainVM.DbInfo.RegisteredMovieCount` 更新、`_registeredMovieCountInitialized` 更新を `Views/Main/MainWindow.MainDbRuntime.cs` に残した。
- `TryAdjustRegisteredMovieCount(...)` の `delta == 0`、DB一致判定、未初期化時の正確値再取得、`Math.Max(0, current + delta)` は MainWindow 側に残し、細かすぎる外出しを避けた。
- `MainWindowFilterSortExecutionPolicyTests` で、登録件数 refresh 結果 guard が helper 経由であることと、helper が `System.Windows` / `Dispatcher` を参照しないことを固定した。

親レビュー:

- 全体観点レビューでは、revision guard と DB一致 guard の意味は `requestRevision == currentRevision && isCurrentDb` に整理されただけで、古い結果やDB切替後結果を弾く契約は維持されると確認した。
- シンプル実装レビューでは、Header count 全体 service 化や `delta == 0` / `Math.Max(...)` の外出しは過剰と判断し、helper を stale guard 1本へ縮小した。
- 親レビューでは、パス比較の正本である `AreSameMainDbPath(...)` を helper 側へ重複実装せず、MainWindow 側で bool 化して渡す判断を維持した。

検証:

- focused test: 196 件成功。
- Release x64 build: 成功。`NETSDK1206` 警告 1 件は既存の SQLitePCLRaw RID 警告。
- `git diff --check`: 成功。
- 更新ドキュメントと新規 helper は UTF-8 BOMなし + LF。
...
## 17. 2026-06-18 Phase 6-C 実施結果

実施内容:

- `Views/Main/SortComboSelectionPolicy.cs` を追加し、sort combo 選択変更時の実行可否、段階ロード中 full reload fallback、サムネ ERROR 順 refresh 要否を WPF 非依存 helper へ分離した。
- `Views/Main/MainWindow.InputRouting.cs` は、選択値の取り出し、`FilterAndSort(...)`、`SortDataAsync(...)`、`RefreshThumbnailErrorRecords(...)`、`SelectFirstItem()` の UI / View 呼び出しを持つ境界として維持した。
- DB未選択、sort combo suppress、表示レコード0件の時は、元の早期 return と同じく `MovieRecs.Count` や選択値文字列化を余分に読まないようにした。
- `Tab_PreviewKeyDown(...)` は `KeyEventArgs` / WPF `Key` / `Tabs` / 既存 shortcut handler 依存が強いため、helper 化しない判断を明示した。
- `MainWindowFilterSortExecutionPolicyTests` で、`FilterAndSort(..., true)` の許容が起動 fallback と段階ロード中 sort の2箇所だけであること、`SortComboSelectionPolicy` が `System.Windows` / `Dispatcher` / UI control を参照しないことを固定した。

親レビュー:

- 全体観点レビューでは、段階ロード中 full reload fallback、通常 sort-only、`id == "28"` の下部 ERROR refresh、`shouldSelectFirstItem` の扱いを維持した。
- シンプル実装レビューでは、`Tab_PreviewKeyDown(...)` を独自 enum / adapter 化しない判断を採用し、`SortComboSelectionPolicy.BuildPlan(...)` 以上へ広げない方針とした。
- 親レビューでは、`FilterAndSort(...)` / `SortDataAsync(...)` / `RefreshThumbnailErrorRecords(...)` / `SelectFirstItem()` を helper へ入れず、View 境界に残す判断を維持した。
...
- Phase 6-D は完了済み。次は抜本 UI 分割ロードマップの完了監査として、`MainWindow.xaml.cs` の残責務、禁止線、source policy、focused test、Release x64 build、実機ログ確認状況を要件単位で照合する。

## 18. 2026-06-18 Phase 6-D 実施結果

実施内容:

- `Views/Main/MainDbSwitchPlanPolicy.cs` を追加し、MainDB 切替時の menu close、旧DB表示状態保存、Recent更新、LastDoc保存、旧Queue pending掃除の実行可否を WPF 非依存 helper へ分離した。
- `Views/Main/MainWindow.DbSwitch.cs` は、DB path 正規化、preflight revision guard、DB schema validation、system table read、`OpenDatafile(...)`、実 side effect 本体を持つ View / runtime 境界として維持した。
- `TrySwitchMainDb(...)` では切替開始時点で `MainDbSwitchSideEffectPlan` を1回作り、pre switch / post switch の両方で同じ plan を参照する形へ寄せた。
- 既存の `ShouldPersistCurrentDbViewStateBeforeSwitch(...)`、`ShouldUpdateRecentFilesOnSuccessfulDbSwitch(...)`、`ShouldRememberLastDocOnSuccessfulDbSwitch(...)`、`ShouldCloseMainMenuBeforeDbSwitch(...)`、`ShouldDiscardPreviousDbPendingThumbnailQueueItemsOnSuccessfulSwitch(...)` は互換 wrapper として残し、既存テストを維持した。
- `IsMainDbSwitchPreflightCurrent(...)` は `_mainDbSwitchPreflightRevision` と `MainVM` を読む後着 guard のため、helper 化しなかった。

親レビュー:

- 全体観点レビューでは、UI起点保存、`StartupAutoOpen` の Recent / LastDoc / menu 抑止、旧Queue pending discard、同一DB / 空DB skip は維持されると確認した。
- シンプル実装レビューでは、`MainDbSwitchSideEffectPlan` は5つの副作用フラグだけを返す純粋 DTO に留まり、DB read / dialog / path I/O / Dispatcher を持たないため許容とした。
- 親レビューでは、preflight guard、`RunMainDbSwitchPreflightAsync(...)`、`TryActivateMainDbSession(...)`、dialog 初期フォルダ系、旧Queue掃除実処理は切らない判断を維持した。
