...
- `Views/Main/MainWindow.xaml.cs` は約 1,936 行まで縮み、起動、dock layout、window lifecycle、キー入力イベントがまだ混在している。
...
- Phase 1 から Phase 3 で、一覧要求制御、表示レコード生成、DB runtime は partial 分離済み。
- 次に起動、dock layout、window lifecycle を分けると、`MainWindow.xaml.cs` は constructor と不可避の event shell にさらに近づく。
- Phase 4 は `MainWindow_ContentRendered(...)`、startup auto open、dock layout 復元、closing drain のログ契約を維持する範囲で小さく進める。
