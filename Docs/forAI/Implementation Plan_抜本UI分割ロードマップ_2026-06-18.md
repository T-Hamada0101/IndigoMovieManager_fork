...
- 候補は Header count refresh policy、Input routing policy のうち、Dispatcher / WPF control / `ObservableCollection` を持ち込まないものに限る。
...
- Phase 6-B として Header count refresh policy または Input routing policy のうち、WPF 非依存で unit test 可能な純粋判断だけを小さく監査する。
