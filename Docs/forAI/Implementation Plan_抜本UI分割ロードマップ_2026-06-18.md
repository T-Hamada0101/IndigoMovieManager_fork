...
- 次もすぐ Core 化するのではなく、WPF 非依存にできる純粋判断だけを小さく選ぶ。
- 候補は DB switch plan DTO などのうち、Dispatcher / WPF control / `ObservableCollection` / DB read を持ち込まないものに限る。該当が薄ければ完了監査へ進む。
...
- Phase 6-C も完了済み。次は Phase 6-D として、DB switch plan DTO などの候補を監査する。
...
- Phase 6-C は完了済み。次は Phase 6-D として DB switch plan DTO などの候補を監査する。ただし DB read / file I/O / Dispatcher / WPF control を持ち込むなら切り出さない。
...
- Phase 6-D として DB switch plan DTO などを監査する。ただし純粋判断だけを小さく切れる場合に限る。候補が薄い場合は、抜本 UI 分割ロードマップの完了監査へ進む。
