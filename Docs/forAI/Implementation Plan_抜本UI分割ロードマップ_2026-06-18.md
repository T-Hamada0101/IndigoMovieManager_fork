...
- 候補は Input routing policy などのうち、Dispatcher / WPF control / `ObservableCollection` を持ち込まないものに限る。
...
- Phase 6-B は完了済み。次は Phase 6-C として、Input routing policy のうち WPF key / control 依存を持ち込まずに切れる純粋判断だけを監査する。
...
- Phase 6-C として Input routing policy を監査する。ただし WPF `Key`、control、Dispatcher を helper へ持ち込むなら切り出さない。キー種別や routing 先の純粋な判定だけが残る場合に限る。
