...
次の実装は Phase 6 の候補監査を推奨する。

理由:

- Phase 1 から Phase 5 で、一覧要求制御、表示レコード生成、DB runtime、起動 / dock layout / window lifecycle、入力 routing は partial 分離済み。
- 次はすぐ Core 化するのではなく、WPF 非依存にできる純粋判断だけを小さく選ぶ。
- 候補は Dock layout validation policy、Header count refresh policy、Input routing policy のうち、Dispatcher / WPF control / `ObservableCollection` を持ち込まないものに限る。
