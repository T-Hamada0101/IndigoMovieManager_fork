...
- ストレスなし操作の UI 分割 v1 として、`FilterAndSortAsync(...)` / `RefreshMovieViewFromCurrentSourceAsync(...)` の ReadModel 計算を `MovieViewReadModelBuilder` へ切り出した。UI 反映は `SortDataAsync(...)` を含めて `TryApplyMovieViewReadModelResultOnUiThread(...)` へ集約し、`request_revision` / `result_count` / `changed` / `update_mode` / `fallback_reason` / `apply_ms` を apply ログで読めるようにした。v1.1 では一覧 snapshot / apply / sort-only UI orchestration を `Views\Main\MainWindow.MovieViewReadModel.cs` partial へ分離し、Phase 1 では ReadModel 要求制御を `Views\Main\MainWindow.MovieViewRequests.cs` partial、Phase 2 では表示レコード生成境界を `Views\Main\MainWindow.MovieRecordFactory.cs` partial、Phase 3 では MainDB runtime 境界を `Views\Main\MainWindow.MainDbRuntime.cs` partial、Phase 4 では起動 / dock layout / window lifecycle 境界を `Views\Main\MainWindow.Lifecycle.cs` と `Views\Main\MainWindow.DockLayout.cs` partial、Phase 5 では入力 / イベント routing 境界を `Views\Main\MainWindow.InputRouting.cs` partial、Phase 6-A では dock layout validation policy を `Views\Main\DockLayoutRestorePolicy.cs` helper へ分離した。`DbSwitch.cs` は切替フロー、`Search.cs` は履歴候補 UI apply として維持し、`FilterAndSort(..., true)` / `Refresh()` / `Items.Refresh()` の許容線は増やしていない。次段は Phase 6-B として、Header count refresh policy または Input routing policy の純粋判断だけを小さく見極める。詳細は `Docs\forAI\Implementation Plan_ストレスなし操作_UI分割_2026-06-18.md` と `Docs\forAI\Implementation Plan_抜本UI分割ロードマップ_2026-06-18.md`。
...
- 2026-06-18 の Release 実機 `debug-runtime.log` 調査では、`first-page shown` / `input ready` は良好で、今回DBの `CreateWatcher` は非支配だった。active skin の `dbinfo-Skin` same-document skip は新形式ログで確認済みのため、残る支配要因候補は過去1件の manual reload deferred scan NullReference と、初回 active skin navigate の `host_navigate_ms` 772.6ms 帯に絞る。
- `CreateWatcher()` / `BuildWatcherCreationPlan(...)` に `availability_ms` / `watch_table_load_ms` / `folder_plan_ms` / `registration_ms` / `apply_ms` の分解計測を追加した。別DBで watcher 作成が再び遅い場合は、この内訳で遅延要因を分類してから最小修正を選ぶ。
...
- active skin の新規実機採取はユーザーDBを直接触らず、コピー `.wb` と `INDIGO_DIAGNOSTIC_NO_PERSIST=1` / `INDIGO_DIAGNOSTIC_STARTUP_DB=<コピー.wb>` を使う。same-document skip を自動確認する時だけ `INDIGO_DIAGNOSTIC_REPEAT_SKIN_REFRESH=1` を併用し、初回 active skin 成功後に同一 `dbinfo-Skin` を 1 回だけ再要求する。診断DB上書きは no-persist 時だけ有効で、LastDoc / Recent / MainWindow / Player / Theme / SettingsWindow の設定保存は skip ログへ閉じる。設定画面や最小 chrome の skin 選択は `system.skin` / profile / LastDoc / Recent を汚し得るため、ユーザーDBでの採取は禁止線とする。
...
- 2026-05-27 のサブ5.5 Worker L で、Everything poll 入口の `Path.Exists(dbPath)` と watch folder 存在確認を背景 plan 化した。UI/入口側では mode / availability / DB path / revision の snapshot だけを取り、`Task.Run` 側で DB path 存在確認、watch folder snapshot 取得、watch folder 存在確認をまとめる。戻り結果は revision / DB path / dispatcher shutdown guard 後にだけ採用し、Worker D の `isDbPathKnownToExist` 引き継ぎを維持して同一 DB path の probe 重複を増やさない。
...
- 2026-05-27 のサブ5.5 Worker I で、watch rename / メニュー rename から入る `RenameThumbAsync(...)` は UI 側で DB path / sort / thumb / bookmark / 対象行 snapshot と表示モデル更新だけを先に行い、DB 更新、サムネ再帰列挙と Move、bookmark jpg 列挙と Move、bookmark DB 更新を `Task.Run` 背景処理へ逃がした。背景結果は rename revision / DB path / dispatcher shutdown guard 後に採用し、最後の一覧反映は既存の `RefreshMovieViewAfterRenameAsync(...)` 経路を維持する。`RenameFile_Click` の実体ファイル `MoveTo` も watcher 抑止の `try/finally` 内で背景化し、抑止復旧を維持する。
- 2026-05-27 のサブ5.5 Worker J で、DB切替 `OpenDatafile(...)` の前段に background preflight を追加し、`TryValidateMainDatabaseSchema(...)` と `system` テーブル読込を旧DB停止前の `Task.Run` へ逃がした。スキーマNGは旧DBを維持したままエラー表示し、preflight 済み `systemData` は `BootNewDb(...)` へ渡して UI スレッド上の同期 SQLite read を避ける。preflight revision と現在DB snapshot がずれた結果は破棄する。
...
10. 済: 実機 `debug-runtime.log` で検索 / sort / watch / 起動 / skin の支配要因を確認した。現行ログでは search / sort / Player / visible-first は主役ではなく、`first-page shown` / `input ready` は良好。2026-06-18 の Release 実機ログでは今回DBの `CreateWatcher` も非支配で、次の候補は active skin の WebView navigate 800〜980ms帯、過去1件の manual reload deferred scan NullReference。
11. 済: `CreateWatcher()` / `BuildWatcherCreationPlan(...)` は `availability_ms` / `watch_table_load_ms` / `folder_plan_ms` / `registration_ms` / `apply_ms` をログへ出し、さらに apply summary へ `attempted` / `failed` / `first_registered_ms` を追加して、別DBで watcher 作成が遅い時だけ支配要因を実機ログで切り分けられるようにした。
