...
変更概要:
- Player音量を`PlayerVolumePolicy`へ一元化し、新規初期値を25%へ変更した。XAML初期化中の`ValueChanged`が保存済み音量を50%で上書きしていたため、設定復元完了までは音量変更イベントを無視する。0〜100%の保存値はそのまま尊重し、不正値だけ25%へ修復する。全画面スナップショットから音量の複製を除き、通常・全画面とも中央状態を参照する。音量・設定保存関連Release x64 57テストと本体buildが成功した。
