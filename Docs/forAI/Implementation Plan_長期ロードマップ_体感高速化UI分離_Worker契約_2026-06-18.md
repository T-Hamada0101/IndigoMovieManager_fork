...
|---|---:|---|---|
| Phase 0. 現状固定とログ証跡補強 | 82% | `UiOperationPriorityPolicy`、ReadModel builder、partial分離、source policy は土台あり。さらに UI Shell / ReadModel Diff / Scheduler / Image / Persistence / Worker / Skin / Player / Watcher の contract source policy を focused test 159件で確認済み。最新run切り出し、run時間窓、run要約、contract / Phase0 evidence 集計、次アクション、監査summary合成 policy、Logタブ preview summary 実出力も focused test 30件で確認済み。search / sort 入力入口の `ui shell input` 出力固定と opt-in live audit test も追加済み。2026-06-27 は contract 完了条件と live audit source policy を固定し、実ログ audit で `log_evidence=2/9`、`phase0_log_evidence=1/12` の不足を確認済み。audit summary は `phase0_audit_complete=true|false` を出し、scroll / manual reload / UI Shell snapshot detail / ReadModel Diff detail / image aggregate / stale discard / Worker detail / Phase7 core route detail の evidence を必須と補助に分けて扱う。さらに Phase2 / Phase3 optional evidence として ReadModel revision / fallback と Scheduler detail を補強し、upper-tab / log-tab 切替入力も optional evidence に加えた。Refresh / Items.Refresh / `FilterAndSort(..., true)` 許容線の広域 source policy も補強済み。optional evidence は `optional_evidence=x/35` で採取量を読める。`DebugRuntimeLog.Write(...)` / `TaskStart(...)` / `TaskEnd(...)` は Release 実機ログ入口として、`Conditional(DEBUG)` / `#if DEBUG` へ戻さない source policy も追加した | 同一 Release run で startup / search / sort / scroll / Player / watch / image / persistence / thumbnail / skin のログを揃える |
| Phase 1. UI Shell 入力契約 | 57% | `UiOperationSnapshot` を追加し、Everything watch / poll と user-priority 判定入口を共通 snapshot 正本へ寄せた。旧 `UiOperationPrioritySnapshot` は互換入口として残すが、MainWindow runtime 側の判定口では使わない。2026-06-25 Worker E で snapshot 共通ログ fields を追加し、user-priority begin / end でも UI Shell 入力状態を同じ語彙で読めるようにした。Worker Averroes で search / sort 入力入口にも `ui shell input` と snapshot fields、Worker Hilbert で `ui_shell_contract=ui-shell-v1` を追加した。2026-06-27 は Player 再生状態の実遷移、PageUp / PageDown 成功時、Header Reload 明示手動再読込、上側タブ切替、Logタブ Debug 切替にも `ui shell input` snapshot を追加した。manual reload とタブ切替は Phase0 必須 token を増やさず optional evidence として監査summaryへ出す | UI event handler を snapshot 生成へさらに寄せ、実機ログで `ui_shell_contract=ui-shell-v1` と search / sort / scroll / player / manual-reload / tab-switch の入口状態を確認する |
...
- 2026-06-27 Worker Franklin: Phase0 summary で optional が1件以上ある時だけ `optional_evidence=x/35` を併記するようにした。採取後に補助 evidence の量を見落としにくくする表示補強であり、必須12件、optional 35件、完了条件は変えていない。
...
- 2026-06-27 Worker Kepler: Logタブ preview の合成テストで latest run に Phase7 optional evidence を含め、先頭summaryに `optional_evidence=x/35 optional=...` が出ることを固定した。production code は変えていない。
...
- 2026-06-27 Worker Curie(Phase0): UI Shell snapshot detail の `is_user_priority_active` / `is_manual_mode` / `is_watch_ui_suppressed` / `is_recent_viewport_active` / `is_player_playback_active` を Phase0 optional evidence として扱う。`ui_shell_contract=ui-shell-v1` と同じ行にある時だけ採用し、Phase0 必須12件は増やさない。optional count は 35 とする。
...
- 2026-06-27 Worker Hypatia: Worker detail optional evidence は `worker_contract=worker-job-v1` と同じ行にある時だけ採用する。汎用 `metric_count` 等の誤検出を避ける補強で、Phase0 必須12件と optional count 35 は変えない。
- 2026-06-27 Worker Descartes: ReadModel Diff detail optional evidence は `diff_contract=readmodel-diff-v1` と同じ行にある時だけ採用する。汎用 diff field の誤検出を避ける補強で、Phase0 必須12件と optional count 35 は変えない。
- 2026-06-27 Worker Hilbert(Image): Image detail optional evidence は `image_contract=image-pipeline-v1` と同じ行にある時だけ採用する。aggregate-decode-plan と stale discard の誤検出を避ける補強で、Phase0 必須12件と optional count 35 は変えない。
- 2026-06-27 Worker Planck: 上側タブ切替入口で `ui shell input: operation_reason=upper-tab-switch` を出す。タブ切替後の snapshot を同じ語彙で読めるようにする。
- 2026-06-27 Worker Lagrange(UI Shell): Logタブ Debug 切替入口で `ui shell input: operation_reason=log-tab-switch` を出す。設定保存と preview 更新の順序は変えない。
- 2026-06-27 Worker Ramanujan: 上側タブ切替 / Logタブ Debug 切替の入力ログを `upper-tab-switch-input` / `log-tab-switch-input` として Phase0 optional evidence に追加した。必須12件は増やさず、optional count は 35 とする。
- 2026-06-27 Worker Poincare: application settings shutdown drain の timeout / failed ログへ `BuildApplicationSettingsWriteRequest(reason).BuildLogFields()` を併記した。待機時間、保存順、例外処理、UI挙動は変えていない。
- 2026-06-27 Worker Carson: thumbnail queue の `failuredb append:` ログへ `BuildWorkerQueueLogFields(...)` 由来の Worker契約語彙を併記した。既存の failuredb 書き込み、handoff 判定、retryable 判定、queue 実行順、IPC方式は変えていない。
- 2026-06-27 Worker SkinRoute: 外部 skin refresh の `refresh end:` に `BuildExternalSkinRefreshCoreLogFields(...)` 由来の core route 語彙を併記した。refresh freshness、same-document skip、Header Reload / fallback retry の意味は変えていない。
- 2026-06-27 Worker PlayerImage: Player右レール stale 破棄ログを `ImageLoadLogFields.Build(loadResult)` へ接続した。同期 decode、stale 判定、UI反映結果は変えず、破棄行だけを画像契約として読めるようにした。
- 2026-06-27 Worker PollScheduler: Everything poll の scheduler 成功経路へ admitted / released ログを追加した。poll 間隔、defer / catch-up 判定、watch scan 入口は変えていない。
- 2026-06-27 Worker WatchScheduler: watch apply の scheduler 成功経路へ core route 付き admitted / released ログを追加した。query-only / full fallback の分岐、ReadModel refresh、`FilterAndSort` 呼び出しは変えていない。
- 2026-06-27 Worker ThumbnailScheduler: ThumbnailProgress snapshot refresh の scheduler 成功経路へ admitted / released ログを追加した。Dispatcher 予約、遅延、完了、失敗ログの意味は変えていない。
- 2026-06-27 Worker PlayerPersistence: Player音量設定保存の shutdown drain timeout / failed ログへ `BuildPlayerVolumeSettingsWriteRequest().BuildLogFields()` を併記した。待機時間、保存順、例外処理、UI挙動は変えていない。
- 2026-06-27 Worker Leibniz(CurrentDbPersistence): 現在DB設定保存の db-changed skip / partial ログへ `BuildCurrentDbSettingsWriteLogFields("settings-window-closing")` を併記した。保存順、`PersistDbSettingsValues(...)`、UI挙動は変えていない。
...
- 親レビューでは、Worker Halley の Phase7 core route detail optional evidence を採用した。skin は `operation_reason=skin.host-refresh` / `definition_mode=`、Player は `player_surface_ready=` / `player_transition=`、Watcher は `watch_apply_kind=` / `watch_reason=` を、対応する `core_route` と同じ行にある時だけ拾う。Phase0 必須12件は増やさず、optional count は後続のタブ切替入力補強込みで 35 として扱う。
...
### 2.34 2026-06-27 PM親レビュー Phase0 summary可読性とReleaseログ入口補強

- Worker Franklin / Feynman は UI簡素化別スレと競合しないよう、Infrastructure policy と DebugRuntimeLog tests に限定した。XAML、Themes、Settings、Views/Main の UI 本体には触れていない。
- 親レビューでは、Worker Franklin の Phase0 summary 可読性補強を採用した。optional evidence が1件以上ある時だけ `optional_evidence=x/35` を `optional=...` の前に出す。optional 0件の summary、Phase0 必須12件、optional 35件、完了条件は変えていない。
- 親レビューでは、Worker Feynman の Release 実機ログ入口 source policy を採用した。`DebugRuntimeLog.Write(...)` / `TaskStart(...)` / `TaskEnd(...)` は reflection で `Conditional(DEBUG)` 不在を見つつ、source 入口近傍でも `Conditional(DEBUG)` / `#if DEBUG` 前提へ戻らないことを固定する。Release 実機ログを設定と絞り込みで制御する契約を守るためのテスト補強であり、production code は変えていない。
- 親検証は focused test 39件成功、Release x64 build 成功、警告0件で完了した。
- 今回も実機同一runの新規採取ではない。次の実機採取では Phase0 summary の `optional_evidence=x/35` が補助 evidence の量を示すことと、Release 実機ログ入口が source policy で守られていることを前提に、引き続き search / sort / scroll / Player / watch / image / thumbnail / skin の同一run不足を閉じる。
...
- 親レビューでは、Worker Kepler の Logタブ preview 補助summary固定を採用した。latest run に optional evidence がある時、Logタブ preview の先頭 `phase0_log_evidence=...` 行にも `optional_evidence=x/35 optional=...` が出ることをテストで固定した。Logタブは引き続き `DebugRuntimeLogAuditSummaryPolicy` 経由で summary を作る。
...
- 今回も実機同一runの新規採取ではない。次の実機採取では live audit の失敗案内に従って startup / search / sort / scroll / Player / watch / image / persistence / thumbnail / skin を同一 Release run で揃え、Logタブ preview の `optional_evidence=x/35` と `phase0_audit_complete=true|false` を確認する。

### 2.36 2026-06-27 PM親レビュー UI Shell detail evidenceと採取案内helper固定

- Worker Boyle(採取案内) / Curie(Phase0) は UI簡素化別スレと競合しないよう、Infrastructure policy と Logタブ / live audit / audit summary tests に限定した。XAML、Themes、Settings、Views/Main UI 本体は触っていない。
- 親レビューでは、Worker Boyle(採取案内) の `BuildFullCaptureGuideText()` を採用した。live audit 失敗案内は `DebugRuntimeLogPhase0NextActionPolicy` の正本順から作り、`startup / search / sort / scroll / Player / watch / image / persistence / thumbnail / skin` の手書き重複を避ける。`phase0_next_actions=...` の挙動は変えていない。
- 親レビューでは、Worker Curie(Phase0) の UI Shell snapshot detail optional evidence を採用した。`ui_shell_contract=ui-shell-v1` と同じ行にある `is_user_priority_active` / `is_manual_mode` / `is_watch_ui_suppressed` / `is_recent_viewport_active` / `is_player_playback_active` だけを拾い、候補 field だけの行は拾わない。Phase0 必須12件は増やさず、optional count は 35 として扱う。
- 親側では Logタブ preview 期待値に残っていた古い optional 総数を `2/35` へ補正した。UI簡素化確認として、MainWindow に `DrawerHost` / `MenuToggleButton` / `MenuConfig` / `MenuTool` が戻っていないこと、設定画面左ペイン化と三ボタン入口が source policy で守られていることも確認した。
- 親検証は focused test 158件成功 / 1件skip、Release x64 build 成功、警告0件で完了した。
- 今回も実機同一runの新規採取ではない。次の実機採取では Logタブ preview / live audit summary の `optional_evidence=x/35` と UI Shell snapshot detail が、同一 Release run の startup / search / sort / scroll / Player / watch / image / persistence / thumbnail / skin 操作後に読めることを見る。
...
- 親レビューでは、Worker Hypatia の Worker detail optional evidence 同行限定化を採用した。`diagnostic_context_count` / `capability_count` / `metric_count` は `worker_contract=worker-job-v1` と同じ行にある時だけ拾い、候補 field だけの行は optional evidence にしない。Phase0 必須12件、optional count 35、完了条件は変えていない。
...
- Phase0 必須12件、optional count 35、完了条件は変えていない。これは実機採取後の summary が汎用 field を拾って誤って観測済みに見えることを避けるための補強である。
...
- Phase0 必須12件、optional count 35、完了条件は変えていない。タブ系入力ログは Phase1 の入力契約補強として扱い、採取手順を重くしない。
...
### 2.40 2026-06-27 PM親レビュー Persistence drainとfailuredb Worker契約補強

- Worker Poincare / Carson は UI簡素化別スレと競合しないよう、`Views/Main/MainWindow.SettingsPersistence.cs` と `src/IndigoMovieManager.Thumbnail.Queue/ThumbnailFailureRecorder.cs`、対応 source policy tests に限定した。XAML、Themes、Settings 画面、UIイベント表面は触っていない。
- 親レビューでは、Worker Poincare の application settings shutdown drain ログ補強を採用した。`application settings save drain timeout/failed` に `persist_contract=persistence-write-v1` / `write_kind` / `write_reason` / `queue_key` / `retryable_policy` が同じ行で乗る。待機時間、保存順、例外処理、UI挙動は変えない。
- 親レビューでは、Worker Carson の `failuredb append:` Worker契約語彙併記を採用した。既存の failuredb handoff ログ1行へ `worker_contract=worker-job-v1`、`worker_stage=completed`、`retryability`、`retryable=false`、`failure_kind` などを `BuildWorkerQueueLogFields(...)` 経由で載せる。failuredb 書き込み、handoff 判定、retryable 判定、queue 実行順、IPC方式は変えない。
- Phase0 必須12件、optional count 35、完了条件は変えていない。今回の補強は Phase5 / Phase6 の実機ログを grep しやすくするための契約同行化であり、実機ログ確認の代替ではない。
- 親検証は focused test 64件成功、Release x64 build 成功、警告0件で完了した。
- 今回も実機同一runの新規採取ではない。次の実機採取では shutdown drain timeout / failed が保存契約行として読めることと、thumbnail terminal failure の `failuredb append:` 行が Worker契約行として読めることを見る。

### 2.41 2026-06-27 PM親レビュー skin refresh endとPlayer右レールstaleログ補強

- Worker SkinRoute / PlayerImage は UI簡素化別スレと競合しないよう、`Views/Main/MainWindow.WebViewSkin.cs`、`UpperTabs/Player/PlayerRightRailImageSourceConverter.cs`、対応 source policy tests に限定した。XAML、Themes、Settings 画面、UIイベント表面は触っていない。
- 親レビューでは、Worker SkinRoute の外部 skin refresh end ログ補強を採用した。`refresh end:` に `core_route=skin-refresh` / `operation_reason=skin.host-refresh` / `refresh_reason` / `request_trace` / `definition_mode` が同じ行で乗り、navigate / same-document skip / host timing の支配要因と route を1行で比較できる。refresh freshness、Header Reload / fallback retry、same-document skip の意味は変えない。
- 親レビューでは、Worker PlayerImage の Player右レール stale 破棄ログ補強を採用した。stale 判定で作った `ImageLoadResult.Canceled(..., "stale-player-right-rail")` を `ImageLoadLogFields.Build(loadResult)` 経由で runtime log へ出し、`image_contract=image-pipeline-v1` / `failure_reason=stale-player-right-rail` / revision を同じ行で読める。同期 decode、stale 判定、UI反映結果は変えない。
- Phase0 必須12件、optional count 35、完了条件は変えていない。今回の補強は Phase4 / Phase7 の実機ログを grep しやすくするための契約同行化であり、実機ログ確認の代替ではない。
- 親検証は focused test 87件成功、Release x64 build 成功、警告0件で完了した。
- 今回も実機同一runの新規採取ではない。次の実機採取では skin `refresh end:` が core route 行として読めることと、Player右レール stale 破棄が画像契約行として optional evidence に乗ることを見る。

### 2.42 2026-06-27 PM親レビュー Scheduler成功経路ログ補強

- Worker PollScheduler / WatchScheduler は UI簡素化別スレと競合しないよう、`Views/Main/MainWindow.xaml.cs`、`Watcher/MainWindow.WatchUiReloadPolicy.cs`、対応 source policy tests に限定した。XAML、Themes、Settings 画面、UIイベント表面は触っていない。
- 親レビューでは、Worker PollScheduler の Everything poll 成功経路ログ補強を採用した。`TryAdmitEverythingWatchPollWork(...)` の accepted + take success で `everything poll scheduler admitted:` と `everything poll scheduler released:` を出し、`UiWorkSchedulerPolicy.BuildAdmissionLogFields(...)` / `BuildTakeLogFields(...)` 経由の `scheduler_contract=scheduler-v1`、`accepted`、`target_index`、`has_request`、`pending_count` を読める。poll 間隔、defer / catch-up 判定、watch scan 入口は変えない。
- 親レビューでは、Worker WatchScheduler の watch apply 成功経路ログ補強を採用した。`watch ui apply scheduler admitted/released:` に `BuildWatchUiApplyCoreRouteLogFields(...)` と scheduler admission / take fields を同居させ、`core_route=watch-ui-apply` と `scheduler_contract=scheduler-v1` を同じ行で grep できる。queue/take 順序、query-only / full fallback 分岐、ReadModel refresh、`FilterAndSort` 呼び出しは変えない。
- Phase0 必須12件、optional count 35、完了条件は変えていない。今回の補強は Phase3 / Phase7 の実機ログを grep しやすくするための成功経路の契約同行化であり、実機ログ確認の代替ではない。
- 親検証は focused test 203件成功、Release x64 build 成功、警告0件で完了した。
- 今回も実機同一runの新規採取ではない。次の実機採取では Everything poll / watch apply の accepted / released / core route が同一 Release run で読めることを見る。

### 2.43 2026-06-27 PM親レビュー ThumbnailProgress schedulerとPlayer音量drain補強

- Worker ThumbnailScheduler / PlayerPersistence は UI簡素化別スレと競合しないよう、`BottomTabs/ThumbnailProgress/MainWindow.BottomTab.ThumbnailProgress.cs`、`Views/Main/MainWindow.Player.cs`、対応 source policy tests に限定した。XAML、Themes、Settings 画面、UIイベント表面は触っていない。
- 親レビューでは、Worker ThumbnailScheduler の ThumbnailProgress snapshot refresh 成功経路ログ補強を採用した。`TryQueueThumbnailProgressSnapshotRefreshWork(...)` の accepted + take success で `snapshot refresh scheduler admitted:` と `snapshot refresh scheduler released:` を出し、`UiWorkSchedulerPolicy.BuildAdmissionLogFields(...)` / `BuildTakeLogFields(...)` 経由の `scheduler_contract=scheduler-v1`、`accepted`、`target_index`、`has_request`、`pending_count` を読める。Dispatcher 予約、遅延 refresh、完了 / 失敗ログ、queue/take 順序は変えない。
- 親レビューでは、Worker PlayerPersistence の Player音量 shutdown drain ログ補強を採用した。`WaitForPlayerVolumeSettingSaveForShutdown(...)` の timeout / failed に `BuildPlayerVolumeSettingsWriteRequest().BuildLogFields()` を併記し、`persist_contract=persistence-write-v1` / `write_reason=player-volume` / `queue_key=player-volume-settings` を同じ行で読める。待機時間、保存順、例外処理、UI挙動は変えない。
- Phase0 必須12件、optional count 35、完了条件は変えていない。今回の補強は Phase3 / Phase5 の実機ログを grep しやすくするための契約同行化であり、実機ログ確認の代替ではない。
- 親検証は focused test 110件成功、Release x64 build 成功、警告0件で完了した。
- 今回も実機同一runの新規採取ではない。次の実機採取では ThumbnailProgress snapshot refresh の admitted / released と、Player音量 drain timeout / failed が保存契約行として読めることを見る。

### 2.44 2026-06-27 PM親レビュー タブ切替optional evidenceと現在DB設定保存契約補強

- Worker Ramanujan / Leibniz(CurrentDbPersistence) は UI簡素化別スレと競合しないよう、Phase0 evidence policy、現在DB設定保存ログ、対応 tests に限定した。XAML、Themes、Settings 画面レイアウト、UIイベント表面は触っていない。
- 親レビューでは、Worker Ramanujan のタブ切替 optional evidence 補強を採用した。`operation_reason=upper-tab-switch` は `upper-tab-switch-input`、`operation_reason=log-tab-switch` は `log-tab-switch-input` として Phase0 summary に出せる。Phase0 必須12件は増やさず、optional count は 35 とする。
- 親レビューでは、Worker Leibniz(CurrentDbPersistence) の現在DB設定保存ログ補強を採用した。db-changed skip / individual settings partial ログに `BuildCurrentDbSettingsWriteLogFields("settings-window-closing")` を併記し、`write_kind=current-db-settings` / `persist_contract=persistence-write-v1` / `queue_key=current-db-settings` を同じ行で読める。保存順、`PersistDbSettingsValues(...)`、DB write 経路は変えない。
- 親検証は focused test 87件成功 / 1件skip、Release x64 build 成功、警告0件で完了した。
- 今回も実機同一runの新規採取ではない。次の実機採取では upper-tab-switch / log-tab-switch が optional evidence に出ることと、現在DB設定保存の skip / partial が保存契約行として読めることを見る。
...
- 済: optional evidence がある Phase0 summary は `optional_evidence=x/35 optional=...` を出す。補助 evidence の採取量を読みやすくするだけで、必須12件と完了条件は変えない。
...
- 済: Logタブ preview でも `DebugRuntimeLogAuditSummaryPolicy` 経由の `optional_evidence=x/35 optional=...` が先頭summaryに出ることをテストで固定した。
...
- 済: Worker detail optional evidence は `worker_contract=worker-job-v1` と同じ行にある時だけ採用する。汎用 field の誤検出を避ける補強であり、Phase0 必須12件と optional count 35 は増やさない。
- 済: ReadModel Diff detail optional evidence は `diff_contract=readmodel-diff-v1` と同じ行にある時だけ採用する。汎用 diff field の誤検出を避ける補強であり、Phase0 必須12件と optional count 35 は増やさない。
- 済: Image detail optional evidence は `image_contract=image-pipeline-v1` と同じ行にある時だけ採用する。aggregate-decode-plan と stale discard の誤検出を避ける補強であり、Phase0 必須12件と optional count 35 は増やさない。
- 済: 上側タブ切替 / Logタブ Debug 切替の `ui shell input` は、`upper-tab-switch-input` / `log-tab-switch-input` として Phase0 optional evidence に残す。タブ切替を採取補助として見るだけで、Phase0 必須12件は増やさない。
...
- 済: 上側タブ切替入口も `ui shell input` と `operation_reason=upper-tab-switch` / `trigger_reason=selection-changed` / `UiOperationSnapshot` fields を `ui-priority` へ出す。Phase0 監査の必須 evidence は増やさず、optional evidence `upper-tab-switch-input` として扱う。
- 済: Logタブ Debug 切替入口も `ui shell input` と `operation_reason=log-tab-switch` / `trigger_reason=debug-switch-changed` / `UiOperationSnapshot` fields を `ui-priority` へ出す。設定保存と preview refresh の順序は変えず、Phase0 optional evidence `log-tab-switch-input` として扱う。
...
- 済: Everything poll、watch apply、ThumbnailProgress snapshot refresh の成功経路も scheduler admitted / released ログを持つ。既存の実行入口は変えず、実機ログで入場成功と次実行引き渡しを同じ scheduler 契約で追える。
...
- 済: Player右レール stale 破棄は `ImageLoadLogFields.Build(loadResult)` 経由で runtime log へ出す。同期 decode と stale 判定の意味は変えず、実機ログでは `failure_reason=stale-player-right-rail` を画像契約行として確認する。
...
- 済: application settings shutdown drain の timeout / failed ログも `BuildApplicationSettingsWriteRequest(reason).BuildLogFields()` を持つ。終了時の保存回収に失敗した場合も、実機ログで保存契約、理由、queue key を同じ行から読める。
- 済: Player音量設定保存の shutdown drain timeout / failed ログも `BuildPlayerVolumeSettingsWriteRequest().BuildLogFields()` を持つ。終了時の音量保存回収に失敗した場合も、実機ログで保存契約、理由、queue key を同じ行から読める。
- 済: 現在DB設定保存の db-changed skip / individual settings partial ログも `BuildCurrentDbSettingsWriteLogFields("settings-window-closing")` を持つ。保存順や DB write 経路は変えず、実機ログで `write_kind=current-db-settings`、保存契約、理由、queue key を同じ行から読める。
...
- 済: thumbnail queue の `failuredb append:` は既存1行のまま Worker契約語彙を併記する。terminal failure の handoff、failuredb 書き込み、retryable 判定、queue 実行順は変えず、救済workerへ渡す親行も `worker_contract=worker-job-v1` で追える。
...
- 済: 外部 skin refresh の `refresh end:` も `BuildExternalSkinRefreshCoreLogFields(...)` 経由の core route fields を持つ。navigate / same-document skip / host timing の重い結果行でも、skin refresh route と支配要因を同じ行で読める。
...
- 済: watch apply の scheduler admitted / released ログは `BuildWatchUiApplyCoreRouteLogFields(...)` と scheduler admission / take fields を同じ行に持つ。実機ログで Watcher core route と scheduler 受理 / 引き渡しを横断して読める。
