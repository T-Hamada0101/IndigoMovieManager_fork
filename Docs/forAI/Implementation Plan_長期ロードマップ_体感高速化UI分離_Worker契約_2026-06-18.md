...
2026-07-12の最上位所見には最小変更を入れた。次回は同じコピーDB + no-persistの新しいRelease runで、連続入力時にTextBoxの文字表示が追従すること、サムネイル進捗が入力後に再開すること、初回検索後の2回目検索で `route=query-only` となり `source-apply` が再発しないことを確認する。ここから先の進捗は、契約数ではなく、ユーザーの待ちと表示の乱れが減ったかで判定する。
