...
- 2026-06-27 Worker Franklin: Phase0 summary で optional が1件以上ある時だけ `optional_evidence=x/33` を併記するようにした。採取後に補助 evidence の量を見落としにくくする表示補強であり、必須12件、optional 33件、完了条件は変えていない。
...
- 2026-06-27 Worker Kepler: Logタブ preview の合成テストで latest run に Phase7 optional evidence を含め、先頭summaryに `optional_evidence=x/33 optional=...` が出ることを固定した。production code は変えていない。
...
- 親レビューでは、Worker Franklin の Phase0 summary 可読性補強を採用した。optional evidence が1件以上ある時だけ `optional_evidence=x/33` を `optional=...` の前に出す。optional 0件の summary、Phase0 必須12件、optional 33件、完了条件は変えていない。
...
- 今回も実機同一runの新規採取ではない。次の実機採取では Phase0 summary の `optional_evidence=x/33` が補助 evidence の量を示すことと、Release 実機ログ入口が source policy で守られていることを前提に、引き続き search / sort / scroll / Player / watch / image / thumbnail / skin の同一run不足を閉じる。
...
- 親レビューでは、Worker Kepler の Logタブ preview 補助summary固定を採用した。latest run に optional evidence がある時、Logタブ preview の先頭 `phase0_log_evidence=...` 行にも `optional_evidence=x/33 optional=...` が出ることをテストで固定した。Logタブは引き続き `DebugRuntimeLogAuditSummaryPolicy` 経由で summary を作る。
...
- 今回も実機同一runの新規採取ではない。次の実機採取では live audit の失敗案内に従って startup / search / sort / scroll / Player / watch / image / persistence / thumbnail / skin を同一 Release run で揃え、Logタブ preview の `optional_evidence=x/33` と `phase0_audit_complete=true|false` を確認する。
...
- 親レビューでは、Worker Curie(Phase0) の UI Shell snapshot detail optional evidence を採用した。`ui_shell_contract=ui-shell-v1` と同じ行にある `is_user_priority_active` / `is_manual_mode` / `is_watch_ui_suppressed` / `is_recent_viewport_active` / `is_player_playback_active` だけを拾い、候補 field だけの行は拾わない。Phase0 必須12件は増やさず、optional count は 33 として扱う。
- 親側では Logタブ preview 期待値に残っていた古い optional 総数を `2/33` へ補正した。UI簡素化確認として、MainWindow に `DrawerHost` / `MenuToggleButton` / `MenuConfig` / `MenuTool` が戻っていないこと、設定画面左ペイン化と三ボタン入口が source policy で守られていることも確認した。
...
- Phase0 必須12件、optional count 33、完了条件は変えていない。今回の補強は Phase5 / Phase6 の実機ログを grep しやすくするための契約同行化であり、実機ログ確認の代替ではない。
...
- Phase0 必須12件、optional count 33、完了条件は変えていない。今回の補強は Phase4 / Phase7 の実機ログを grep しやすくするための契約同行化であり、実機ログ確認の代替ではない。
...
- Phase0 必須12件、optional count 33、完了条件は変えていない。今回の補強は Phase3 / Phase7 の実機ログを grep しやすくするための成功経路の契約同行化であり、実機ログ確認の代替ではない。
...
- Phase0 必須12件、optional count 33、完了条件は変えていない。今回の補強は Phase3 / Phase5 の実機ログを grep しやすくするための契約同行化であり、実機ログ確認の代替ではない。
...
- 済: optional evidence がある Phase0 summary は `optional_evidence=x/33 optional=...` を出す。補助 evidence の採取量を読みやすくするだけで、必須12件と完了条件は変えない。
