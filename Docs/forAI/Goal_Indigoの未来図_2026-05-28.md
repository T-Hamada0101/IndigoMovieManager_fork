...
変更概要:
- 再構築した長期ロードマップに合わせ、未来図の中心を「処理が速い」から「入力を待たせず、表示を乱さず、最新の意思を守り、背後処理が譲る」状態へ明確化した。
- 完了判断を Behavior / Evidence / Regression Guard の3条件へ揃え、主要8シナリオの同一Release runと実表示確認を未来図の受け入れ基準へ追加した。
...
この文書は、UI分離とスムーズ表示を含むIndigo全体の上位ゴールである。

ただし、日々の実装順は `Implementation Plan_UIを含む高速化のための抜本改善プラン_2026-04-17.md` を優先する。未来図は判断基準、UI抜本改善Planは着手順、長期ロードマップは体感目標とPhase Gateとして併読する。
...
- 入力受理、選択、focus、scroll、blank、ちらつき、後着巻き戻りを、処理時間と同じ重さで扱う。
- 完了は Behavior、Evidence、Regression Guard の3条件が揃った時だけ認める。

### 2.1 ストレスなし操作の完成像

Indigoが目指すのは、すべての仕事を同時に最速で終わらせることではない。ユーザーが操作している時に、必要な仕事が正しい順番で進むことである。

- クリック、選択、scroll、tab、Player操作はすぐ受理される。
- 250msを超える明示操作は、進行中か継続利用可能かが分かる。
- 一覧更新で選択、focus、scroll位置を不必要に失わない。
- 読み込み途中に意味のないblank、全件消去、ちらつきを見せない。
- search、sort、watch、image、skinの古い結果が最新表示を巻き戻さない。
- watch、poll、thumbnail、rescue、catalog、保存はユーザー操作へ譲り、操作終了後に安全に再開する。
- エラー回復と難読動画対応は、通常動画の初動を重くしない。
...
- 主要8シナリオを同一Release runで追い、入力受理と最初の有用表示の間にある支配要因を最大3件へ絞れる。
- 一覧更新後も選択、focus、scroll位置が保たれ、blank、ちらつき、後着巻き戻りが増えていない。
- 250msを超える明示操作が無反応に見えず、操作中も利用可能なUI領域が不必要に塞がれない。
- Behavior、Evidence、Regression Guard の3条件が同じ変更系列で揃っている。
...
ログは高速化の飾りではない。未来図では、ログで説明できない改善は完了扱いにしない。同時に、ログ項目や契約識別子を追加しただけでも完了扱いにしない。実表示のBehaviorと回帰検出まで揃える。
...
- 主要8シナリオの同一Release runで支配要因を絞ってから着手しているか。
