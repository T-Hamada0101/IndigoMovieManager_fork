...
変更概要:
- 2026-07-12、scroll user-priority中のvisible source image probe要求を単一workerの最新reasonへ畳み、延期受付時に旧viewport revisionをstale化、解除後に最新snapshotを作り直すようにした。Release x64コピーDBのPageDown 8回で、旧runのstale完了7件は0件、`first_render_ms=28 max_layout_gap_ms=355 total_ms=658`。解除後に同じ63件probeが2回続くため、次順位はwarm / viewport由来の同一snapshot重複と、描画計測より長い `activity=None` 監視値の因果監査である。
