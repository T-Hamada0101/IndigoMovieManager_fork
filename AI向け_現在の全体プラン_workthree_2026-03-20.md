...
- 2026-07-12、Player scroll burstとUI hangログへ同じ `burst_id` を付け、heartbeatをBackground飢餓ではなくInput応答で測るようにした。同一fingerprintのvisible source probeは正常完了後2秒抑止し、scroll中warm enqueueも抑止してidle revision flushへ譲った。実機で `queue_enqueued_count=0 suppressed_count=40` は確認したが、PageDown 8回は `max_layout_gap_ms=2053 total_ms=3583` で改善せず、warmは主因ではない。次順位はuser-priority開始前から動くサムネWorker・外部decodeのCPU競合とcontainer生成を分けて監査する。人間の物理ホイール確認は未達。
