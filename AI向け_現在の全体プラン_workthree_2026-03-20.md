...
- ストレスなし操作の UI 分割は v1 / v1.1 から Phase 6-B まで進み、ReadModel 計算、一覧 apply、要求制御、表示レコード生成、MainDB runtime、起動 / dock layout / window lifecycle、入力 routing を専用 partial / helper へ分けた。Phase 6-A では `Views\Main\DockLayoutRestorePolicy.cs`、Phase 6-B では `Views\Main\RegisteredMovieCountRefreshPolicy.cs` を追加し、WPF 非依存の純粋判断だけを切り出した。`DbSwitch.cs` は切替フロー、`Search.cs` は履歴候補 UI apply として維持し、`FilterAndSort(..., true)` / `Refresh()` / `Items.Refresh()` の許容線は増やしていない。次段は Phase 6-C として、Input routing policy のうち WPF key / control 依存を持ち込まない純粋判断だけを監査する。詳細は `Docs\forAI\Implementation Plan_ストレスなし操作_UI分割_2026-06-18.md` と `Docs\forAI\Implementation Plan_抜本UI分割ロードマップ_2026-06-18.md`。
